                    ...
                    <ul class="metismenu" id="menu">
                        <li><a class="has-arrow ai-icon" href="javascript:void(0)" aria-expanded="false">
                            ...
                            <span class="nav-text">Dashboard</span>
                                ...
                                <li><button @click="redirect()">Dashboard</button></li>
                ...
                <div class="row">
                    ...
                    <div class="col-xl-12">
                        <v-card>
                            ...
                            <v-data-table @click:row="clickTable"
                                          :headers="headers"
                                          :items="data"
                                          :search="search"
                                          id="example" class="display min-w850"
                            ></v-data-table>
                    </v-card>
                    </div>
                </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers:[
                    ...
                    { text: 'Type de poste', value: 'TypePoste' },
                ],
                search: '',
            }
        },
        methods: {
            async getData(){
                ...
                var d = [];
              await axios.get('http://127.0.0.1:8000/NonTermineAffectation').then(function (res){
                  d=JSON.parse(res.data)
              })

                var i = 0;
              for(i;i<d.length;i++){
                  //var dateDebut = new Date(d[i].dates[0][0].timestamp * 1000);
                  console.log(d[i].dates)
                  var dates = JSON.parse(d[i].dates)

                  var dateDebut = dates[0][0]
                  var dateFin = dates[dates.length -1][1]

                  this.data.push({"id":d[i].id,"TypePoste":d[i].typePoste,"user":d[i].user.username,"projet":d[i].projet.nomProjet,"poste":d[i].poste.poste,"dateDebut":dateDebut,"dateFin":dateFin})
              }
            },
            async clickTable(items){
                this.stagiaire = false;
                this.freelancer = false;
                this.other = false;
                this.idAffectation = items.id
                this.projet = items.projet;
                this.poste = items.poste;
                this.typeProjet =items.TypePoste;
                if(this.typeProjet ==="Stagiaire"){
                    this.stagiaire = true;
                    this.tache = "";

                }
                if(this.typeProjet ==="Freelance"){
                    this.freelancer = true;
                    this.tache = "";
                }
                if(this.typeProjet ==="Permanant" || this.typeProjet ==="Intermittant"){
                    this.other = true;
                }
                ...
                var pos = [];
                var poste = {"poste":this.poste};
                const options = {
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                };
                await axios.post("http://127.0.0.1:8000/GetPosteByName",poste,options).then(function (res) {
                    ...
                    //console.log(d)
                    let listOfObject = Object.keys(d).map((key) => {
                        return d[key]
                    })
                    //console.log(typeof (this.categorie))
                    var k = 0;
                    for(k;k<listOfObject.length;k++){
                        pos.push({"id":listOfObject[k].id,"poste":listOfObject[k].poste,"salaireMinimaile":listOfObject[k].salaireMinimaile,"tache":listOfObject[k].tache,"categorie":listOfObject[k].categorie.categorie,"secteur":listOfObject[k].secteur.secteur,"filiere":listOfObject[k].filiere.filiere,"field":listOfObject[k].field.field});
                    }
                    //this.posteInformations = listOfObject[0];
                })

                this.posteInformations = pos;
                console.log(this.posteInformations)

                if(this.posteInformations.length === 2){
                    var s1  = parseFloat(this.posteInformations[0].salaireMinimaile.replace(",","."));
                    var s2 = parseFloat(this.posteInformations[1].salaireMinimaile.replace(",","."));
                    console.log(s1)
                    if(s1 > s2){
                        this.salaireMaximale = this.posteInformations[0].salaireMinimaile
                        this.salaireMinimaile = this.posteInformations[1].salaireMinimaile
                    }else{
                        this.salaireMinimaile = this.posteInformations[0].salaireMinimaile
                        this.salaireMaximale = this.posteInformations[1].salaireMinimaile
                    }

                }else{
                    this.salaireMinimaile = this.posteInformations[0].salaireMinimaile
                    this.salaireMaximale =  null;
                }
          ...
          redirect(){
            ...
            change(){
                if(this.posteInformations.length === 2){
                    var salaireMin = parseFloat(this.salaireMinimaile.replace(",","."))
                    var salaireMax = parseFloat(this.salaireMaximale.replace(",","."))
                    var salaire = parseFloat(this.salaire.replace(",","."))

                    if(salaire<= salaireMax && salaire >=salaireMin){
                        if(salaireMin === parseFloat(this.posteInformations[0].salaireMinimaile.replace(",","."))){
                            alert(this.posteInformations[0].categorie)
                            this.categorie = this.posteInformations[0].categorie
                            this.secteur =  this.posteInformations[0].secteur
                            this.filiere = this.posteInformations[0].filiere
                            this.field = this.posteInformations[0].field
                            this.tache = this.posteInformations[0].tache
                            this.id = this.posteInformations[0].id
                        }
                        if(salaireMin === parseFloat(this.posteInformations[1].salaireMinimaile.replace(",","."))){
                            alert(this.posteInformations[1].categorie)
                            this.categorie = this.posteInformations[1].categorie
                            this.secteur =  this.posteInformations[1].secteur
                            this.filiere = this.posteInformations[1].filiere
                            this.field = this.posteInformations[1].field
                            this.tache = this.posteInformations[1].tache
                            this.id = this.posteInformations[1].id
                        }
                    }else{
                        if(salaire>salaireMin){
                            this.categorie = this.posteInformations[0].categorie
                            this.secteur =  this.posteInformations[0].secteur
                            this.filiere = this.posteInformations[0].filiere
                            this.field = this.posteInformations[0].field
                            ...
                            alert("Salaire ne peut pas être inférieure à "+salaireMin)
                        }
                    }
                }
                else{
                    var salaireMini = parseFloat(this.salaireMinimaile.replace(",","."))
                    var salairee = parseFloat(this.salaire.replace(",","."))
                    if(salairee >=salaireMini){
                        this.categorie = this.posteInformations[0].categorie
                        this.secteur =  this.posteInformations[0].secteur
                        this.filiere = this.posteInformations[0].filiere
                        this.field = this.posteInformations[0].field
                        ...
                        alert("Salaire ne peut pas être inférieure à "+salaireMini)
                    }
                }
            ...
            async Save(){
                ...
                var d = {"id":this.id,"idAffectation":this.idAffectation,"salaire":this.salaire,"tache":this.tache,"tuteur":this.tuteur}
                await axios.post('http://127.0.0.1:8000/AddSalaire',d,options).then(res=>console.log(res.data))
                console.log(d)
            }
        },
        beforeMount(){
